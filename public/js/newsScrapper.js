...
$(document).ready(function() {
	...
	$(".searchArticle").on("click", function() { // Scrap Articles Request
		...
		fetch("/api/search", {method: "GET"}).then(function(){ window.location.replace("/api/search");});
	...
	$(".addArticle").on("click", function(element) { // Save an Article Request
		...
		fetch("/api/add", { // Send savedArticle to the Server
			...
			setTimeout(function(){ $("#modalMessage").modal("close"), 1500;});
	...
	$(".savedArticles").on("click", function()  { // Query for Saved Articles
		...
		fetch("/api/savedArticles", {method: "GET"}).then(response, function(){ response.json();}).then((response), function() {
  
			response.map(article, function() {
				...
				$(document.getElementById(article["_id"])).on("click", function(event) { // Event Listenr For Saved Article
					...
					}).then(response, function() { response.json();}).then((data), function() {
						console.log("fetching data");
						console.log("data is " + JSON.stringify(data));
						$(".boxComments").html("");
						if (data.length >= 1) {
							data.map((comment), function() {
								...
								if (comment === null) {
									...
									notesDiv = "<div class='col s12 m7' id='" + comment["_id"] + "'><div class='card horizontal'><div class='card-image'><img src='https://lorempixel.com/100/190/nature/6'></div><div class='card-stacked center'><div class='card-content valign-wrapper'><p>" + comment.body + "</p></div><div class='card-action deleteComment' data-id=" + comment["_id"] + "><a href='#'>Delete</a></div></div></div></div>";
									$(".boxComments").prepend(notesDiv);
								}
				...
				$(".deleteArticle").on("click", function(event) { // Event Listenr For Saved Article Delete Button
					...
					fetch("/api/deleteArticle", { // Send savedArticle to the Server
						...
						setTimeout(function() { $("#modalMessage").modal("close"), 2000;
